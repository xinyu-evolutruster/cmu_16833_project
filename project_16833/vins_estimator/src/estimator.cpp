...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 5, 1>>>> &image, const map<int, vector<pair<int, Vector4d>>> &lines, const std_msgs::Header &header)
{
    ...
    if (f_manager.addFeatureCheckParallax(frame_count, image, lines))
...
void Estimator::LineBAincamera()
{
    ...
    for (auto &it_per_id : f_manager.linefeature)
    {
        ...
        for (auto &it_per_frame : it_per_id.linefeature_per_frame)
        {
            ...
            if (imu_i == imu_j)
            {
                lineProjectionFactor_instartframe *f = new lineProjectionFactor_instartframe(obs);
                problem.AddResidualBlock(f, loss_function,
                                         para_LineFeature[linefeature_index]);
            }
            else
            {
                lineProjectionFactor_incamera *f = new lineProjectionFactor_incamera(obs);
                problem.AddResidualBlock(f, loss_function,
                                         para_Pose[imu_i],
                                         para_Pose[imu_j],
                                         para_Ex_Pose[0],
