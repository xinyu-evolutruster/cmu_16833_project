                          ...
                          std::vector<DMatch> good_matches)
{
    ...
    string path = "/home/cindy/16833/project/catkin_PLVINS/src/PL-VINS/feature_tracker/src/image/";
    name = path + name + ".jpg";
    frame_num++;
...
void LineFeatureTracker::readImage(const cv::Mat &_img)
{
    ...
    opts.canny_th1 = 50.0;
    opts.canny_th2 = 120.0;
    opts.canny_aperture_size = 5;
    opts.do_merge = false;
    ...
    opts.length_threshold = opts.min_length;
    ...
    sum_detection_time += t_li.toc();
    counter++;
    mean_detection_time = sum_detection_time / counter;

    ROS_INFO("line detect costs: %fms", t_li.toc());
    ROS_INFO("Average line detection cost: %lfms", mean_detection_time);
